...
The installed plugin now contains a packaged runtime executable rather than a
loose Python dependency tree:
...
`main.py` is the Decky bridge. `bin/decktation-runtime` owns controller
monitoring, audio capture, transcription, and the private `ydotoold` process.
...
If you need to rebuild the packaged runtime locally before creating the Decky
ZIP, use:
...
3. Start its private runtime-managed `ydotoold`
4. Ready to use!
...
# Runtime controller helper logs (via plugin log)
...
- Read `doc/RUNTIME_REFACTOR.md` for the runtime architecture
